...
def render_shapes(shapes, screen, dz, tuning, opacity=1.0):
    shape_states = []
    min_z = 300
    for shape in shapes:
    ...
    # Render origin points first (separate loop)
    for state in shape_states:
        shape = state['shape']
        p = state['origin_vertex']
        if hasattr(shape, 'hastext') and shape.hastext:
            text = shape.getText(p)
            g = 127 + round(shape.ov[p][2] * tuning)
            if text:
                pygame.draw.circle(screen, (g,g,g), (WIDTH//2 + round(shape.ov[p][0]), HEIGHT//3 + round(shape.ov[p][1])), 10)
                text_surf = font.render(text, True, (255, 255, 255))
                screen.blit(text_surf, (WIDTH//2 + round(shape.ov[p][0]-5), HEIGHT//3 + round(shape.ov[p][1]-5)))
            ...
            while i < len(edges):
                ...
                if numpy.array_equal(e.at, e.target):
                    ...
                    if e.target_index in state['remaining_vertices']:
                        ...
                        # Render text for reached vertices if shape has text
                        if hasattr(state['shape'], 'hastext') and state['shape'].hastext:
                            text = state['shape'].getText(e.target_index)
                            if text:
                                pygame.draw.circle(screen, (color_val,color_val,color_val), (WIDTH//2 + round(e.at[0]), HEIGHT//3 + round(e.at[1])), 10)
                                text_surf = font.render(text, True, (255, 255, 255))
                                screen.blit(text_surf, (WIDTH//2 + round(e.at[0])-5, HEIGHT//3 + round(e.at[1])-5))
                    continue
                i += 1
            ...
            for current_cell_idx in list(cellhl):
                ...
                for loop in layerpoly:
                    if len(loop) > 2:
                        ...
                        # Use an alpha value of 15 per-slice for a cumulative transparent volume effect
                        alpha = int(15 * opacity)
                        color = (int(base[0] * intensity), int(base[1] * intensity), int(base[2] * intensity), alpha)
...
while running:
    ...
    render_shapes(shapes, screen, dz, tuning, opacity)
