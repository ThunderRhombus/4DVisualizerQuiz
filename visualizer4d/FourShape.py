...
class FourShape:
    ...
    def _rebuild_ov(self):
        """Reproject rv -> ov using current ortho value."""
        self.ov = []
        for p in self.rv:
            x, y, z, w = p[0], p[1], p[2], p[3]
            d = self.o * w
            self.ov.append(numpy.array([x*(1+d), y*(1+d), z*(1+d), w]))
    ...
    def rotate(self, yaw, pitch, roll, dip, tuck, skew):
        ...
        for p in self.v:
            x, y, z, w = p[0], p[1], p[2], p[3]
            self.rv.append(A @ numpy.array([x, y, z, w]))
    ...
    def shrink(self, ortho):
        self.o = ortho
        # Re-project with new ortho if we have rotated vertices
        if self.rv:
            self._rebuild_ov()
