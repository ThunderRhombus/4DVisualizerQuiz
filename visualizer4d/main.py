...
# ============================================================
#  BLACK SCREEN FIX — paste this block immediately after all
#  try_import(...) calls and before TOTAL_QUIZ_QUESTIONS = 15
# ============================================================

# Collect any failed critical imports so we can show a useful error screen
_CRITICAL = {
    'Tesseract':          Tesseract,
    'Cube':               Cube,
    'ThreeAxis':          ThreeAxis,
    ...
    'WShellRenderer':     WShellRenderer,
    'CellHlRenderer':     CellHlRenderer,
    'ToggleButton':       ToggleButton,
    'OriginRenderer':     OriginRenderer,
}
_FAILED_IMPORTS = [name for name, obj in _CRITICAL.items() if obj is None]

# ── If any critical module failed, show an error screen and exit ──────────────
if _FAILED_IMPORTS:
    ...
    # Replace the normal main so pygbag runs _show_error instead
    async def main_async():
        await _show_error()

# ── End of import-error guard ─────────────────────────────────────────────────
# The real main_async() definition continues below (unchanged).
# If _FAILED_IMPORTS is empty, none of the above runs.
...
class Slider:
    ...
    def _hit(self, pos):
        hx = self._val_to_px()
        return math.hypot(pos[0] - hx, pos[1] - self.y) <= self.HANDLE_R + 4
    ...
    def draw(self, surface, font):
        ...
        pygame.draw.rect(surface, self.COLOR_TRACK,
                         (self.x, self.y - self.TRACK_H//2, self.w, self.TRACK_H), border_radius=2)
        fw = self._val_to_px() - self.x
        if fw > 0:
            pygame.draw.rect(surface, self.COLOR_FILL,
                             (self.x, self.y - self.TRACK_H//2, fw, self.TRACK_H), border_radius=2)
        hx  = self._val_to_px()
...
# ============================================================
#  Simple dropdown for shape selection
# ============================================================
class Dropdown:
    """A compact dropdown for selecting from a list of named options."""
    BG       = (40,  40,  60)
    BG_OPEN  = (50,  50,  75)
    BG_HOT   = (70,  70, 100)
    BG_SEL   = (80, 140, 220)
    ...
    def draw(self, surface, font):
        ...
        # Desaturate slightly for the header background
        ...
        # Arrow
        ax = hr.right - 14; ay = hr.centery
        d  = 4
        if self.open:
            pts = [(ax-d, ay+d//2), (ax+d, ay+d//2), (ax, ay-d//2)]
        else:
            pts = [(ax-d, ay-d//2), (ax+d, ay-d//2), (ax, ay+d//2)]
...
async def submit_survey_to_google_form(user_id, source, familiarity):
    form_url = "https://docs.google.com/forms/d/e/YOUR_FORM_ID/formResponse"
    data = {"entry.888888": str(user_id), "entry.999999": source, "entry.101010": str(familiarity)}
    if "YOUR_FORM_ID" in form_url:
        return
    try:
        if sys.platform == 'emscripten':
...
async def main_async():
    ...
    try:
        ...
        FREE_HUD  = 170   # taller HUD for three rows
        SLIDER_W  = 220
        ...
        tutorial_btns = [
            ToggleButton(0, 0, 200, 38, TUTORIAL_BTN_LABELS[i], TUTORIAL_BTN_COLS[i])
            for i in range(3)
        ]
        ...
        quiz_buttons = [ToggleButton(0, 0, 160, 30, f"Option {i+1}", (150,150,150))
                        for i in range(5)]
        btn_idk = ToggleButton(0, 0, 160, 30, "I Don't Know", (200, 100, 100))

        # ------------------------------------------------------------------
        # Free-mode selector buttons (mode only — shape via dropdown)
        # ------------------------------------------------------------------
        free_mode_btns = [
        ...
        def layout_quiz():
            ...
            for i, b in enumerate(quiz_buttons):
                b.rect.x = bx + i * 174
                b.rect.y = HEIGHT - 68
            btn_idk.rect.x = bx + 5 * 174
            btn_idk.rect.y = HEIGHT - 68
        ...
        def make_pool(o):
            ...
            for cls, size, name in constructors:
                try:
                    pool.append(cls(size, o, 0, 0, 0, 0))
                except Exception as e:
                    print(f"[4D] make_pool: {name} failed — {e}", flush=True)
                    import traceback; traceback.print_exc()
                    # Append None as placeholder so indices stay stable
                    pool.append(None)
            return pool
        ...
        def rebuild_free_shape():
            ...
            # Find first valid shape at or after free_shape_idx
            shape = pool[free_shape_idx] if free_shape_idx < len(pool) else None
            if shape is None:
                print(f"[4D] rebuild_free_shape: pool[{free_shape_idx}] is None, trying fallbacks", flush=True)
                shape = next((p for p in pool if p is not None), None)
            if shape is None:
                print("[4D] rebuild_free_shape: ALL shapes failed to construct!", flush=True)
                return
            active_shape = shape
            try:
        ...
        def setup_question():
            ...
            if qi == 0 or qi == 2:
                ...
                active_shape = random.choice(pool[1:])
            else:
                active_shape = random.choice(pool)
            axes_shape  = ThreeAxis(75, ortho, 0, 0, 0)
        ...
        while running:
            try:
                ...
                # FIX: pending_shape_idx tracks dropdown changes so we can apply
                # them after the event loop without nonlocal assignment issues inside
                # a nested scope.
                ...
                # FIX: Apply pending shape dropdown change here, in the outer scope
                # where free_shape_idx lives, so no nonlocal gymnastics are needed.
                ...
                try:
                    ...
                    # ---- TUTORIAL ----
                        ...
                        if tutorial_sub == "WATCH":
                            ...
                            for i, (iy2, ip2, ir2) in enumerate(indicator_params):
                                ...
                                screen.blit(lbl, (ox2 + ORIG_SIZE//2 - lbl.get_width()//2,
                                                  oy_base + ORIG_SIZE + 4))
                    ...
                    # ---- INTERSTITIAL ----
                        ...
                        layout_quiz()   # <-- FIX: ensures btn_continue rect is positioned
                    ...
                    # ---- QUIZ STATES ----
                        ...
                        layout_quiz()
                        yaw -= dy; roll -= dr
                        if not paused and all(a4_variants[i] is not None for i in range(6)):
                            for i in range(6):
                        ...
                        if state == "ANALYSIS":
                            ...
                            # Five origin indicators across bottom
                    ...
                    # ---- FREE MODE ----
                        ...
                        # Shape dropdown (draw last so it overlaps other HUD elements when open)
                        ...
                        sh_lbl = small_font.render("4D Spin  (drag handles or click track)",
                                                   True, (200,200,255))
